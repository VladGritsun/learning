...
def experiment():
    ...
    for i in range(1000):
        ...
        s = Stat()
        e_in = s.error(points,actual_line,learned_line)
        new_points = g.generatePoints(100, i_range)
        e_out = s.error(new_points,actual_line,learned_line)
        in_error.append(e_in)
        out_error.append(e_out)
    print "IN ERROR " + str(sum(in_error)/len(in_error))
    print "OUT ERROR " + str(sum(out_error)/len(out_error))
